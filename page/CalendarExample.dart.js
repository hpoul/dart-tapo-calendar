...
$$.HashMapImplementation = {"":
 ...
 _ensureCapacity$0: function() {
  ...
  if ($.gtB(this._numberOfDeleted, numberOfFree))
    this._grow$1($.get$length(this._lib0_keys));
},
          ...
          switch (state) {
            ...
            case 2:
              ...
              if (existingKey == null) {
                ...
                return hash;
              else if ($.ltB(insertionIndex, 0) && $.CTC9 === existingKey)
                insertionIndex = hash;
              var numberOfProbes0 = numberOfProbes + 1;
              hash = $.HashMapImplementation__nextProbe(hash, numberOfProbes, $.get$length(this._lib0_keys));
            case 3:
...
$$.HashSetIterator = {"":
 ...
 hasNext$0$bailout: function(state, t1, t2) {
  if ($.geB(t1, $.get$length(t2)))
    return false;
  if ($.index(t2, this._nextValidIndex) === $.CTC9)
    this._advance$0();
...
$$.Calendar = {"":
 ["_timeFrameHeight", "_hourMultiplier", "_minuteFactor", "_hourHeight", "_dayds", "wrapper"],
 super: "Object",
 _updateEventDisplays$0: function() {
  var events = this._dayds.getEvents$0();
  var eventWrapper = $._Elements_DivElement();
  $.add$1(eventWrapper.get$classes(), 'cal-eventlistwrapper');
  var t1 = $.iterator(events);
  var t2 = this._hourMultiplier;
  if (typeof t2 !== 'number')
    return this._updateEventDisplays$0$bailout(1, t2, t1, eventWrapper, 0, 0);
  var t4 = this._minuteFactor;
  var t5 = this._timeFrameHeight;
  if (typeof t5 !== 'number')
    return this._updateEventDisplays$0$bailout(2, t2, t4, t1, eventWrapper, t5);
  for (; t1.hasNext$0() === true;) {
    var t3 = t1.next$0();
    var elEvent = $._ElementFactoryProvider_Element$html('<div class="cal-event ui-corner-all ui-droppable">\n  <div class="cal-event-titlebar">\n    <span class="cal-event-time">09:40 - 12:24</span>\n    <span class="cal-event-title">Example Title</span>\n    <!--a style="float: right;"><img src="/media/saas/images/openerp/listgrid/delete_inline.gif"></a-->\n  </div>\n  <div class="cal-event-body-wrapper">\n    <input type="text" class="cal-event-body">\n  </div>\n  <div class="cal-event-resize"></div>\n</div>\n');
    var start = this._formatTime$1(t3.get$start());
    var end = this._formatTime$1(t3.get$end());
    var t6 = $.S(start) + ' - ' + $.S(end);
    elEvent.query$1('.cal-event-time').set$text(t6);
    t6 = t3.get$title();
    elEvent.query$1('.cal-event-title').set$text(t6);
    t6 = t3.get$body();
    elEvent.query$1('input.cal-event-body').set$value(t6);
    t3.get$start();
    var quarters = $.toInt($.round($.add($.mul(t3.get$start().get$hour(), t2), $.div(t3.get$start().get$minute(), t4))));
    var endquarters = $.toInt($.round($.add($.mul(t3.get$end().get$hour(), t2), $.div(t3.get$end().get$minute(), t4))));
    $.print($.S(t3.get$start().get$hour()) + ', ' + $.S(t3.get$start().get$minute()) + ' - ' + $.S(t3.get$end().get$hour()) + ' /// ' + $.S($.div($.add($.mul(t3.get$start().get$hour(), t2), t3.get$start().get$minute()), t4)) + ' -- startquarters: ' + $.S(quarters) + ' end: ' + $.S(endquarters));
    var top$ = $.toInt($.mul(quarters, t5));
    var height = $.toInt($.mul($.sub(endquarters, quarters), t5));
    $.print('timeframeheight: ' + $.S(t5));
    t6 = $.S(height) + 'px';
    elEvent.get$style().set$height(t6);
    t6 = $.S(top$) + 'px';
    elEvent.get$style().set$top(t6);
    $.add$1(eventWrapper.get$nodes(), elEvent);
  }
  $.add$1(this.wrapper.query$1('.cal-datescol').get$nodes(), eventWrapper);
},
 _updateEventDisplays$0$bailout: function(state, env0, env1, env2, env3, env4) {
  switch (state) {
    case 1:
      t2 = env0;
      t1 = env1;
 ...
 _formatTime$1: function(date) {
  var t1 = date.get$hour();
  if (typeof t1 !== 'number')
    return this._formatTime$1$bailout(1, date, t1, 0);
  var hour = t1 < 10 ? '0' + $.S(date.get$hour()) : $.S(date.get$hour());
  t1 = date.get$minute();
  if (typeof t1 !== 'number')
    return this._formatTime$1$bailout(2, date, hour, t1);
  var minute = t1 < 10 ? '0' + $.S(date.get$minute()) : $.S(date.get$minute());
  return hour + ':' + minute;
},
 _formatTime$1$bailout: function(state, env0, env1, env2) {
  switch (state) {
    case 1:
      var date = env0;
      t1 = env1;
  ...
  switch (state) {
    ...
    case 1:
      state = 0;
      var hour = $.ltB(t1, 10) ? '0' + $.S(date.get$hour()) : $.S(date.get$hour());
      t1 = date.get$minute();
    case 2:
      state = 0;
      var minute = $.ltB(t1, 10) ? '0' + $.S(date.get$minute()) : $.S(date.get$minute());
      return hour + ':' + minute;
  }
},
 _init$0: function() {
  var el = $._ElementFactoryProvider_Element$html('<div class="tapo-calendar">\n  <div class="cal-wrapper">\n    <table class="cal-table">\n      <tr>\n        <td class="cal-timecol-placeholder"></td>\n        <td>\n          <div class="cal-timegrid-wrapper">\n            <div class="cal-timegrid">\n              <!-- this will include the time grid (the background grid) -->\n            </div>\n          </div>\n        </td>\n      </tr>\n      <tr>\n        <td class="cal-timecol">\n          <!-- this will include the time labels -->\n        </td>\n        <td class="cal-datescol">\n          <!-- this will include the actual date events. -->\n        </td>\n      </tr>\n    </table>\n  </div>\n</div>\n');
  var timecol = el.query$1('.cal-timecol');
  var timegrid = el.query$1('.cal-timegrid');
  for (var i = 0; i < 24; ++i) {
    var hourLabel = $._ElementFactoryProvider_Element$html('<div class="cal-timelabel-wrapper"><div class="cal-timelabel">' + $.S(i < 10 ? '0' + $.S(i) : $.toString(i)) + ':00</div></div>');
    $.add$1(timecol.get$nodes(), hourLabel);
    var gridLine = $._ElementFactoryProvider_Element$html('<div class="cal-timegrid-hour"><div class="cal-timegrid-hour-half"></div></div>');
    $.add$1(timegrid.get$nodes(), gridLine);
  }
  $.add$1(this.wrapper.get$nodes(), el);
  this._updateEventDisplays$0();
},
 init$1: function(dayds) {
  this._dayds = dayds;
  this._init$0();
},
 Calendar$1: function(wrapper) {
  var t1 = this._minuteFactor;
  if (typeof t1 !== 'number')
    throw $.iae(t1);
  this._hourMultiplier = 60 / t1;
  var t2 = this._hourHeight;
  var t3 = this._hourMultiplier;
  if (typeof t3 !== 'number')
    throw $.iae(t3);
  this._timeFrameHeight = t2 / t3;
}
};
...
$$.SimpleDayDatasource = {"":
 ["_day", "_events"],
 super: "DayDatasource",
 getEvents$0: function() {
  return this._events;
}
};

$$.SimpleEvent = {"":
 ["_body", "_title", "_lib1_end", "_lib1_start"],
 super: "CalendarEvent",
 get$body: function() {
  return this._body;
},
 ...
 SimpleEvent$4: function(start, end, title, body) {
  this._lib1_start = start;
  this._lib1_end = end;
  this._title = title;
  this._body = body;
}
...
$$._WorkerSendPort_send_anon = {"":
 ...
 call$0: function() {
  ...
  if ($._globalState0().get$isWorker() === true)
    ...
    $.index($._globalState0().get$managers(), this.this_2.get$_workerId()).postMessage$1(workerMessage);
}
};
...
$$._NativeJsSendPort_send_anon = {"":
 ...
 call$0: function() {
  ...
  var isolate = $.index($._globalState0().get$isolates(), this.this_5.get$_isolateId());
  if (isolate == null)
    return;
  if (this.this_5.get$_receivePort().get$_callback() == null)
    return;
  var shouldSerialize = !($._globalState0().get$currentContext() == null) && !$.eqB($._globalState0().get$currentContext().get$id(), this.this_5.get$_isolateId());
  ...
  $._globalState0().get$topEventLoop().enqueue$3(isolate, new $._NativeJsSendPort_send_anon0(this.this_5, t1, shouldSerialize), 'receive ' + $.S(this.message_4));
}
};
...
$.SimpleDayDatasource$ = function(_day, _events) {
  return new $.SimpleDayDatasource(_day, _events);
};
...
$.getFunctionForTypeNameOf = function() {
  ...
  if ($.contains$1(userAgent, $.CTC11) === true)
    return $.typeNameInChrome;
...
$.prepareDayDatasource = function() {
  var now = $.DateImplementation$now();
  var today = $.DateImplementation$(now.get$year(), now.get$month(), now.get$day(), 0, 0, 0, 0, false);
  var events = $.ListFactory_List(null);
  events.push($.SimpleEvent$(today.add$1($.DurationImplementation$(0, 5, 0, 0, 0)), today.add$1($.DurationImplementation$(0, 6, 0, 0, 0)), 'First Event', 'With some descriptive text. (Scroll down for the second event ;) )'));
  events.push($.SimpleEvent$(today.add$1($.DurationImplementation$(0, 9, 0, 0, 0)), today.add$1($.DurationImplementation$(0, 11, 0, 0, 0)), 'Test Event', 'Some long description about this event'));
  return $.SimpleDayDatasource$(today, events);
};
...
$.main = function() {
  $.Calendar$($.query('div#placeholder')).init$1($.prepareDayDatasource());
  $.query('body p').set$text('Hello World!');
};
...
$.Calendar$ = function(wrapper) {
  var t1 = new $.Calendar(null, null, 15, 60, null, wrapper);
  t1.Calendar$1(wrapper);
  return t1;
};
...
$.SimpleEvent$ = function(start, end, title, body) {
  var t1 = new $.SimpleEvent(null, null, null, null);
  t1.SimpleEvent$4(start, end, title, body);
  return t1;
};
...
$.CTC10 = new Isolate.$isolateProperties.ConstantMap(Isolate.$isolateProperties.CTC0, {}, 0);
...
$.$defineNativeClass('HTMLAppletElement', ["name?", "height!"], {
 is$Element: function() { return true; }
});
...
$.$defineNativeClass('Element', ["style?", "title?", "innerHTML!", "id?"], {
...
$.$defineNativeClass('HTMLEmbedElement', ["name?", "height!"], {
 is$Element: function() { return true; }
});
...
$.$defineNativeClass('HTMLIFrameElement', ["name?", "height!"], {
 is$Element: function() { return true; }
});

$.$defineNativeClass('HTMLImageElement', ["name?", "height!"], {
 complete$1: function(arg0) { return this.complete.call$1(arg0); },
 is$Element: function() { return true; }
});

$.$defineNativeClass('HTMLInputElement', ["value=", "pattern?", "name?", "height!"], {
 is$Element: function() { return true; }
});
...
$.$defineNativeClass('HTMLObjectElement', ["name?", "height!"], {
 is$Element: function() { return true; }
});
